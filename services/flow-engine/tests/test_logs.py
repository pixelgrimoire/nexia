...
def test_handle_message_logs_trace_id(monkeypatch, caplog):
    # monkeypatch redis.xadd so no external service needed
    ...
    # find any log record with trace_id in its dict
    found = any(getattr(r, 'trace_id', None) for r in caplog.records)
    assert found, f"expected trace_id in log records, got: {[r.getMessage() for r in caplog.records]}"
